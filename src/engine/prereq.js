...
function canCastOf(character, db) {
  ...
  const race = character?.species ? resolveRaceObj(db, character.species.id, character.species.source) : null;
...
export function prereqContext(character, { db = null, grantedFeatures = [] } = {}) {
  ...
  const raceObj =
    db && character?.species ? resolveRaceObj(db, character.species.id, character.species.source) : null;
...
const TEXTS = { level: levelText, ability: abilityText, race: raceText };
