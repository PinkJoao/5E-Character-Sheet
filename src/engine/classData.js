...
export function parseClass(classObj) {
  ...
  const skillBlock = classObj.startingProficiencies?.skills?.[0]?.choose;
  const skillChoice = skillBlock
    ? { from: (skillBlock.from ?? []).map(skillCode), count: skillBlock.count ?? 0 }
    : { from: [], count: 0 };
